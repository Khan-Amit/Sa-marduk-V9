#!/usr/bin/env python3
# sa_akhran_core.py – The 36‑Group Logic Engine
...
# Load external threat data
DATA_DIR = os.path.join(os.path.dirname(__file__), 'threat_data')
...
MALICIOUS_EXTS = load_json('malicious_extensions.json')
RANSOMWARE_HASHES = load_json('ransomware_hashes.json')
BOOT_WHITELIST = load_json('boot_whitelist.json')
...
def voice_of_brahma(text, group):
    ...
    # Load from external JSON as well
    for ext_group, exts in MALICIOUS_EXTS.items():
        for ext in exts:
            if text.lower().endswith(ext):
                mal_words.append(ext.replace('.', ''))
    
    mal = sum(1 for w in mal_words if w in lower)
    safe = sum(1 for w in safe_words if w in lower)
    
    if group['trait'] in ['Root-Trust Only', 'Hash-Anchored']:
        safe += 2
    if group['trait'] == 'Bayesian-Predictive':
        mal += 1
    
    # Check ransomware hashes
    for h in RANSOMWARE_HASHES.get('known_ransomware', []):
        if h in text:
            mal += 5
